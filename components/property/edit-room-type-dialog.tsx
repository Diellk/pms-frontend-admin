...
import type { RoomType, UpdateRoomTypeRequest } from "@/lib/types/property"
...
export function EditRoomTypeDialog({ roomType, open, onOpenChange, onSuccess }: EditRoomTypeDialogProps) {
  ...
  useEffect(() => {
    if (roomType) {
      ...
      setSize(roomType.size?.toString() || "")
      setBedType(roomType.bedType || "")
      setNumberOfBeds(roomType.numberOfBeds?.toString() || "")
      setSelectedAmenities(roomType.amenities || [])
      setImages(roomType.images || [])
      setActive(roomType.active)
      setError(null)
    }
  }, [roomType])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true)
      await propertyApi.updateRoomType(roomType.id, request)
      onOpenChange(false)
      onSuccess()
    } catch (err) {
